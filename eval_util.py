...
def getEvalArgs(parser=None):
	...
	run_settings = json.load(open("resources/%s/run_settings.json" % args.run_id))
...
from concrete_classes import TFLiteClassifier, FlairClassifier

def get_classifier(project, run_dir, run_id, model_version, model=None, base_dir="./"):
	if project == "modelmaker":
		return TFLiteClassifier(run_id, model_version, base_dir=base_dir, model=model)
	elif project == "flair":
		return FlairClassifier(run_dir, model_version)
	else:
		raise Exception("unknown project, cannot get classifier for project `%s`" % project)
...
def getSegments(chat):
	...
	for i, ct in enumerate(chat["content"] + [{"type": "separator"}]):
		if ct is None or ct.get("type", "default_type") != "separator": continue
...
def getWarningLatency(nonempty_messages, skepticism):
	...
	for i, msg in enumerate(nonempty_messages):
		if "prediction" not in msg:
			# print("skipped %s" % msg)
			continue
		mc_raised_warning = mc.add_prediction(msg["prediction"] >= 0.5)
...
def isNonemptyMsg(msg):
	...
	return msg is not None and msg.get("type", "default") == "message" and bool(msg.get("body", ""))
...
def breakUpChat(chat, args, eval_mode=None):
	...
	if eval_mode.startswith("segments"):
		...
		return filter(lambda s: isGood(s, args.dataset_indicator),
		              getSegments(chat))
